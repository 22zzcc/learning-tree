...
export function demoTreeSpec(title: string, reason: string): DemoNodeSpec {
  return {
    name: title,
    definition: title + '：本次学习目标。演示模式下生成的是通用骨架，配置 API Key 后 AI 会按你的摸底结果生成个性化知识树。',
    ...
    children: [
      {
        name: '「' + title + '」的基本概念',
        definition: '围绕「' + title + '」最常用的术语、组成部分与基本规则。',
...
export async function seedDemoIfEmpty(): Promise<void> {
  ...
  const specs = [shortDivision, economics, swimming]
  const nodes: TreeNode[] = []
  const queue: { spec: DemoNodeSpec; lineId: string; parentId: string | null }[] = []
  lines.forEach((l, i) => queue.push({ spec: specs[i], lineId: l.id, parentId: null }))
